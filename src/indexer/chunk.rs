use crate::compressors::compressor::{CompressionAlgorithm, Compressor};
...
pub struct Chunk {
    ...
    pub compressed_positions: Vec<u8>, // stored on disk
...
impl Chunk {
    ...
    pub fn index_positions(&mut self) {
        if self.compressed_positions.len() == 0 {
            return;
        }
        ...
        while i < self.positions.len() {
            let mut j = i;
            while self.compressed_positions[j] != 0 {
                j += 1;
            }
            posting_list = &self.compressed_positions[i as usize..j as usize];
            i = j + 1;
    ...
    pub fn add_positions(&mut self, positions: Vec<u32>) {
    ...
    pub fn decode(&mut self, chunk_bytes: &[u8]) {
        ...
        self.compressed_doc_ids = chunk_bytes[offset..index].to_vec();
        self.compressed_positions = chunk_bytes[index + 1..].to_vec();
